...
class RpmPM(PackageManager):
    ...
    def _invoke_dnf(self, dnf_args, fatal = True, print_output = True ):
        ...
        # Try to find dnf in STAGING_BINDIR_NATIVE first, then fall back to PATH
        staging_bindir_native = self.d.getVar('STAGING_BINDIR_NATIVE')
        dnf_cmd = None
        if staging_bindir_native:
            dnf_path = os.path.join(staging_bindir_native, "dnf")
            if os.path.exists(dnf_path) and os.access(dnf_path, os.X_OK):
                dnf_cmd = dnf_path
        if not dnf_cmd:
            dnf_cmd = bb.utils.which(os.getenv('PATH'), "dnf")
        if not dnf_cmd:
            bb.fatal("Could not find dnf command in PATH or STAGING_BINDIR_NATIVE")
