...
do_compile:append() {
	...
	# Set serial baudrate to 115200 in defconfig before compilation
	if [ -f "configs/${UBOOT_MACHINE}" ]; then
		sed -i 's/^CONFIG_BAUDRATE=.*/CONFIG_BAUDRATE=115200/' "configs/${UBOOT_MACHINE}"
		if ! grep -q "^CONFIG_BAUDRATE=" "configs/${UBOOT_MACHINE}"; then
			echo "CONFIG_BAUDRATE=115200" >> "configs/${UBOOT_MACHINE}"
		fi
	fi
